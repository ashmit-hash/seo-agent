...
Open with the occasion and a specific, visual scenario. No clichés. No generic openers.
The hook must anchor: occasion name + brand name + a specific product use case or recipient type.
End the hook with a natural bridge into the product picks.
...
List 3–6 products. For each product, write exactly:
- **[Product Name]** — ₹[exact price]
- [1–2 sentences: specific reason-to-buy that names a use case, recipient type, or occasion moment. No generic praise.]
${urgencyLine ? `\nAdd this urgency line at the TOP of this section:\n"${urgencyLine}"` : ""}

### SECTION 3 — BUYER'S GUIDE
2–3 practical tips for choosing the right gift from this category. Specific to the occasion and recipient type. No generic "consider your budget" filler.
...
${policyContext ? `USE THESE BRAND POLICY EXCERPTS to write at least 2 brand-specific Q&As (shipping time, returns, care, or customisation drawn from policy text). The other 1–2 may be category or product-specific. ZERO purely generic FAQs are allowed.

BRAND POLICY EXCERPTS:
${policyContext}` : "Write Q&As a real buyer would search. At least 1 must be occasion or care-specific to this product."}

Each answer MUST be exactly 2 complete prose sentences, each 20–30 words, totalling 40–60 words.
...
BANNED PHRASES — zero uses anywhere in the blog (delete on sight):
...
- "true testament" / "stands as a testament" / "is a testament to" / "a testament to"
...
VOCABULARY CAPS — count uses in your draft before outputting. Exceed the cap → replace with specific product/material/use-case language:
- "premium" → max 3 uses
- "elegant" / "sophisticated" → max 2 uses each
- "timeless" → max 2 uses
- "crafted" → max 3 uses
- "luxurious" / "refined" / "elevated" / "exceptional" → max 2 uses each
- "exquisite" / "meticulously" / "impeccable" / "outstanding" → max 1 use each
