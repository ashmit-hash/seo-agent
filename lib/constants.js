...
- Strip these before using the name in the blog.
...
- Fix automatically before using in the blog.
...
✔ Matches the blog topic category
...
Each answer MUST be 40-60 words minimum (this is the length Google pulls for featured snippets and People Also Ask boxes — short answers never get picked up).
Write complete, genuinely helpful answers.
BANNED FAQ openers: "Great question!", "You're not alone in wondering this..."
...
## STEP 5 — QUALITY CHECK (Do this before outputting)

[ ] Does the blog's primary angle match the brand's actual product category — not a generic lifestyle blog?
[ ] Is the tone consistent with the brand's voice — not a generic salesy template?
[ ] Are ALL product names and prices ONLY in Section 7 — nowhere else in the blog?
[ ] Is every piece of advice specific and visual — no vague phrases like "subtle is key"?
[ ] Do all product names match the product list exactly — no invented names?
[ ] Do all prices match the product list exactly — no rounded or estimated prices?
[ ] Does every FAQ answer have 40+ words?
[ ] Is the hook free of clichés?
[ ] Is "Alippo" absent from the entire blog?
[ ] Are any prompt artifact headings present ("Modified Keyword Integration", "SEO Section", "Step X")? → DELETE them
[ ] Does the blog end with a complete CTA — not mid-sentence?
[ ] DUPLICATE OCCASION CHECK: Does Section 4 repeat any occasion already covered by a named combo in Section 3? → If yes, DELETE the duplicate paragraph entirely. Do not rewrite or merge it.
[ ] BRAND NAME FREQUENCY CHECK: Does the brand name appear 3-4 times in the blog body (excluding product section and CTA)? → Count them. If fewer than 3 — insert naturally into existing sentences.
[ ] BRAND NAME IN CTA CHECK: Does the CTA OPEN with the brand name — not bury it at the end of the sentence? → If buried, rewrite the CTA opening line.
