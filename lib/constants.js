...
- If selling artificial/fashion/imitation jewellery — state that explicitly. Do NOT classify as fine jewellery.
- If no prices found — state "Price Range: Not found" and infer the category (budget / mid / premium) from the brand tone.
...
Now confirm these four things internally before writing:

1. "This website is about: [identify the actual category — food, fashion, SaaS, tiffin service, lifestyle, artificial jewellery, etc.]"
...
4. "The maximum product price on this website is: [extract from PRICE RANGE line above, or infer from scrape data]"
...
If the website sells artificial / imitation jewellery — write about artificial jewellery. Do NOT treat it as fine jewellery.
...
🚨 RULES:
- If the website context shows a maximum price (e.g. MAX ₹800), then EVERY price you write in the blog MUST be below that maximum.
- If the brand sells products under ₹800 — do NOT write ₹2000, ₹5000, ₹10,000 or any higher amount anywhere.
- Budget brackets must match the brand's actual range. Example: if max is ₹800, use brackets like "Under ₹200 / Under ₹400 / Under ₹800" — NOT "Under ₹2000 / Under ₹5000".
- If no price data is found — use CONSERVATIVE price ranges. For artificial/fashion jewellery default to under ₹500. For handmade/artisan products default to under ₹1000.
- NEVER invent premium price brackets for a budget brand.

Price check: Read the website context. Find the price range. Lock it. Do not exceed it anywhere in the blog.
...
Before you output the blog, check every item:
...
- Does ANY price in this blog exceed the brand's maximum price from the website context? → If yes, REPLACE with correct lower prices
- Are the price brackets realistic for THIS brand (not generic market prices)? → If no, FIX them
- If the brand sells artificial/fashion jewellery, are you using artificial jewellery language (NOT "gold", "silver", "Kundan", "fine jewellery")? → If no, REWRITE
...
Approved Structure:
${blueprintStructure || "Hook → Explain simply → Why it matters → Options → Tips → Mistakes → FAQ → CTA"}

Follow this structure section by section. Do not skip. Do not add extra sections.
...
- Sentences: maximum 12 words each
