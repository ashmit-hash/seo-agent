...
export const STEPS = [
  ...
  { id: 5, label: "Validation Checkpoint",  icon: "shield-check",  short: "Validate"    },
...
// Runs between Blueprint (4) and Blog Post (6).
// Verifies catalog, brand identity, topic alignment, prices, internal links,
// and artifacts. Emits a locked_snapshot the Blog Post step writes from.
// Hard blocks stop the workflow; soft warnings surface but don't block.

export const PROMPT_VALIDATE_SYSTEM = `You are the Validation Checkpoint agent in BlogIQ. You run between the Blueprint step and the Blog Post step. Your single job is to produce an immutable, verified data snapshot that the Blog Post step will write from. You do not write content. You verify, lock, and either pass through or block.

You must execute every validation block below. Continue all checks even if some fail — produce a full report. Hard blocks (block status) stop the workflow. Soft warnings (warn status) surface in the UI but do not block.

Your output is a single JSON object. No prose outside the JSON. No markdown fences around the JSON. No commentary.

You never write blog content. You never modify the Blueprint. You never invent products, prices, or URLs. If a catalog lookup fails, it fails — do not fabricate. The product_catalog is the single source of truth for products and prices. The brand_config is the single source of truth for brand identity.`.trim();

export const PROMPT_VALIDATE = ({
  brandName, brandTone, brandVoice, bannedPhrases, priceFormat, allowsHinglish,
  productCatalog, approvedTopic, targetKeyword,
  blueprintText, batchHistory, festivalContext, currentDate,
  blockAMode = "block",
}) => {
  const brandConfig = JSON.stringify({
    name: brandName || "Unknown Brand",
    tone: brandTone || "warm and helpful",
    voice_attributes: brandVoice || [],
    banned_phrases: bannedPhrases || [],
    signature_phrases: [],
    allows_hinglish: allowsHinglish !== false,
    price_format: priceFormat || "₹{amount}",
  });

  const catalogJson = typeof productCatalog === "string"
    ? productCatalog
    : JSON.stringify(productCatalog || []);

  const historyJson = typeof batchHistory === "string"
    ? batchHistory
    : JSON.stringify(batchHistory || []);

  const festiveJson = festivalContext
    ? (typeof festivalContext === "string" ? festivalContext : JSON.stringify(festivalContext))
    : "[]";
...
<approved_topic>${approvedTopic || ""}</approved_topic>
<target_keyword>${targetKeyword || ""}</target_keyword>

<blueprint>
${blueprintText || "(no blueprint provided)"}
</blueprint>
...
<current_date>${currentDate || new Date().toISOString().split("T")[0]}</current_date>

---

# Validation Procedure

Execute the following blocks in order. For each check, record: pass, warn, or block. Continue all checks even if some fail.

## Block A — Catalog Resolution
IMPORTANT: The Blueprint at this stage may be a content STRUCTURE OUTLINE (section headings, content type, target reader) rather than a full draft with product names. This is expected — products are injected during blog generation. Treat Block A failures as "${blockAMode}" (${blockAMode === "warn" ? "WARNING only — do not block" : "BLOCK — stop generation"}).
1. For every product name explicitly mentioned in the Blueprint: look up by name in product_catalog. No match → ${blockAMode}: hallucinated_product.
2. Check for repeat-digit patterns (e.g. "925925") in product names → block: concatenation_artifact (always a hard block).
3. If the same product appears 3+ times in batch_history → warn: product_overused.
4. Count successfully resolved products. If fewer than 3 AND Blueprint contains actual product names → ${blockAMode}: insufficient_catalog_match. If Blueprint is a structure outline with no product names yet → warn only, do not block.
5. If Blueprint references a product class not in catalog → ${blockAMode}: product_class_mismatch.

## Block B — Brand Identity Lock
1. Resolve canonical brand_name from brand_config.name.
2. Scan Blueprint for unfilled placeholders: "Brand Name", "{{brand}}", "[brand]", "[Brand]", "[Insert Brand]", "BRAND" → block: unfilled_placeholder.
3. Verify all brand name occurrences match canonical casing exactly → block: brand_variant if any variant found.
4. If allows_hinglish is false, check for Devanagari/non-Latin scripts → block: foreign_language_leak.
5. Attach tone, voice_attributes, banned_phrases to snapshot.

## Block C — Topic ↔ Catalog Alignment
1. Topic must not contain the brand name → block: branded_topic.
2. Topic must not equal a single product name → block: product_as_topic.
3. Topic must map to 3+ products in catalog by keyword overlap → block: topic_catalog_mismatch if fewer.
4. If topic references a festival, verify festival date is on or after current_date → warn: past_festival if not.
5. target_keyword must overlap with at least one product category → warn: weak_keyword_alignment if not.

## Block D — Internal Link Pre-binding
1. For each product in Blueprint, resolve pdp_url as /products/{handle} from catalog.
2. For each collection/category mentioned, resolve /collections/{handle}.
3. Total resolved links < 5 → block: insufficient_links.
4. Build internal_links_map: { entity_name: url }.

## Block E — Price Snapshot Lock
1. For each resolved product, pull price from product_catalog.
2. Format using brand_config.price_format.
3. Product without price → block: missing_price.
4. Numeric token in Blueprint that resembles a price but has no product_id → block: floating_price.
5. Lock all prices into snapshot.

## Block F — Artifact & Placeholder Pre-scan
1. Repeat-digit patterns via regex (\\d+)\\1+ → block: concatenation_artifact.
2. Unfilled placeholders: "{{", "}}", "[brand]", "[product]", "<insert>", "TODO", "TBD", "XXX", "___" → block: unfilled_placeholder.
3. Prompt-leak phrases: "I don't have", "lack of context", "cannot be listed", "as an AI", "based on the information provided", "without more information", "I'm unable to", "due to the lack of" → block: prompt_leak.
4. H1 must appear exactly once; no skipped heading levels (H1 → H3 without H2) → block: heading_hierarchy.
5. FAQ section must have at least 4 Q&A pairs → warn: faq_incomplete if not.
6. Generic AI openers: "In today's fast-paced world", "Look no further", "When it comes to", "Dive into", "Discover the world of", "Elevate your", "In conclusion", "Without further ado", "Whether you're a", "From [X] to [Y]" → warn: generic_ai_pattern.
...
If all hard blocks pass:
{
  "status": "pass",
  "validation_summary": {
    "blocks_passed": ["A","B","C","D","E","F"],
    "warnings": [{ "block": "string", "type": "string", "detail": "string" }]
  },
  "locked_snapshot": {
    "approved_topic": "string",
    "target_keyword": "string",
    "brand_profile": {
      "name": "string", "tone": "string", "voice_attributes": [],
      "banned_phrases": [], "signature_phrases": [], "allows_hinglish": true,
      "price_format": "₹{amount}"
    },
    "locked_products": [
      { "id": "string", "name": "string", "price": "string", "pdp_url": "string", "category": "string" }
    ],
    "locked_collections": [{ "id": "string", "name": "string", "url": "string" }],
    "internal_links_map": { "entity_name": "url" },
    "festive_context": null,
    "constraints": {
      "min_word_count": 1200, "required_internal_links": 5,
      "required_faq_pairs": 4, "banned_phrases_global": []
    }
  }
}

If any hard block failed:
{
  "status": "block",
  "failures": [
    {
      "block": "string", "failure_type": "string", "detail": "string",
      "affected_text": "string", "remediation": "string"
    }
  ],
  "warnings": [{ "block": "string", "type": "string", "detail": "string" }]
}

Run the validation now.`.trim();
};

// ─── Post-generation validation (Step 6 → 7 gate) ────────────────────────────
export const PROMPT_POST_VALIDATE = (blogText, lockedSnapshot) => `
Given the generated blog post below and the locked_snapshot, verify:

1. PRODUCTS: Every product name in the body matches a name in locked_snapshot.locked_products. Flag any product mentioned that is not in the list.
2. PRICES: Every price in the body matches the corresponding locked_products[*].price exactly. Flag any drift.
3. LINKS: Every product/collection mention in the body has an <a href> matching internal_links_map. Flag missing links.
4. BRAND NAME: brand_profile.name appears in canonical form everywhere. Flag variants.
5. ARTIFACTS: Re-run Block F scans — placeholders, prompt-leaks, heading hierarchy, FAQ count (min 4), generic AI patterns.

LOCKED SNAPSHOT:
${JSON.stringify(lockedSnapshot || {}, null, 2)}

BLOG POST:
${blogText}

Return ONLY:
{ "status": "pass" | "fail", "failures": [{ "check": "string", "detail": "string", "affected_text": "string" }] }
`.trim();
...
ANTI-SCRAPING RULE — The brand name above is the ONLY valid source.
Never derive or substitute the brand name from:
  - The website's HTML <title> tag
  - Any meta-og:title, meta-description, or Open Graph tag
  - The H1, H2, or any heading on the scraped page
  - The domain name (e.g. samika.co → "Samika" is NOT acceptable)
  - Any other scraped page element

Test: Complete this sentence using the brand name variable above:
"[BRAND_NAME] offers a stunning collection of..."

— If you can replace [BRAND_NAME] with the actual brand name from the variable → proceed.
— If the variable is empty, blank, or still reads as "[BRAND_NAME]" → STOP.
  Return exactly: "ERROR: Brand name variable is empty. Please check variable injection before generating."
— If the variable contains any of these generic words:
  "Jewellery Website", "Silver Jewellery", "Fashion Store", "Online Store",
  "My Store", "Our Store", "Store", "Shop", "Website", "Brand", "Company"
  → treat it as empty. STOP.
  Return exactly: "ERROR: Brand name appears to be a platform default, not a real brand name. Please provide the correct brand name before generating."

Do NOT generate any blog content if the brand name fails this test.
Do NOT guess, infer, or extract the brand name from any other source.

${lockedSnapshot ? `================================================================
LOCKED SNAPSHOT — SINGLE SOURCE OF TRUTH (from Validation Checkpoint)
You MUST use ONLY the data below. Violating any constraint = output rejected.
================================================================

APPROVED TOPIC: ${lockedSnapshot.approved_topic || choice}
TARGET KEYWORD: ${lockedSnapshot.target_keyword || ""}
BRAND NAME (canonical): ${lockedSnapshot.brand_profile?.name || brandName}
PRICE FORMAT: ${lockedSnapshot.brand_profile?.price_format || "₹{amount}"}
TONE: ${lockedSnapshot.brand_profile?.tone || ""}

LOCKED PRODUCTS (use ONLY these — exact names, exact prices):
${(lockedSnapshot.locked_products || []).map(p => `• ${p.name} — ${p.price} | link: ${p.pdp_url}`).join("\n")}

INTERNAL LINKS MAP (every product/collection mention must use these URLs):
${JSON.stringify(lockedSnapshot.internal_links_map || {})}

CONSTRAINTS:
- Min word count: ${lockedSnapshot.constraints?.min_word_count || 1200}
- Required internal links: ${lockedSnapshot.constraints?.required_internal_links || 5}
- Required FAQ pairs: ${lockedSnapshot.constraints?.required_faq_pairs || 4}
- Additional banned phrases: ${(lockedSnapshot.constraints?.banned_phrases_global || []).join(", ") || "see rules below"}

HARD RULE: If you cannot satisfy these constraints with the data above → return:
{ "status": "blog_generation_blocked", "reason": "specific reason" }
Do NOT improvise, invent, or approximate. The snapshot is immutable.
...
RULE 1 — BRAND NAME ZERO TOLERANCE + GRAMMAR
...
BRAND NAME GRAMMAR (critical — this is the #1 AI-tell):
Brand names are PROPER NOUNS, not countable nouns. Never use them like a product noun.
WRONG: "eat a [Brand]" / "are [Brand] suitable" / "a [Brand]" / "how long before yoga should I eat a [Brand]?"
RIGHT: "[Brand] snacks" / "are [Brand]'s products suitable" / "[Brand]'s range" / "how long before yoga should I eat a [Brand] snack?"
Test before writing any sentence with the brand name: Can you insert "a" or "an" directly before the brand name? If yes → rewrite. Nobody says "eat a Nike" or "are Apple suitable."
...
RULE 5 — NO DUPLICATE OCCASION CONTENT (STRICT ENFORCEMENT)

UNDERSTAND THE DIFFERENCE BETWEEN THE TWO SECTIONS:
  Section 3 — Outfit Combinations
    Purpose: Show specific clothing + jewellery pairings
    Focus: WHAT to wear together (visual, specific)
    Trigger: Outfit type (casual dress, office blazer, saree, jeans)
  Section 4 — Occasion-Based Styling
    Purpose: Show when and where to wear the jewellery
    Focus: CONTEXT and SETTING (event, location, activity)
    Trigger: Life event (wedding, college, festival, beach, office meeting, date night)
  These two sections MUST NEVER overlap. If an occasion appears in Section 3 → it CANNOT appear in Section 4, even with different wording.

OCCASION MASTER LIST — build this BEFORE writing:
  Occasions used in Section 3 (Outfit Combinations): [fill as you write]
  Occasions used in Section 4 (Occasion-Based Styling): [must be DIFFERENT from all above]
  Rule: once used in either section, an occasion CANNOT appear in the other.
  "Casual day out" and "running errands" = SAME occasion. Only one section can use it.

OCCASION ALLOCATION RULE — plan BEFORE writing:
  Good example:
    Outfit Combo 1 → Office wear (uses: Office)
    Outfit Combo 2 → Casual bohemian (uses: Casual)
    Outfit Combo 3 → Evening glamour (uses: Evening)
    Occasion Section 1 → Wedding (NEW)
    Occasion Section 2 → College campus (NEW)
  Wrong example (NEVER do this):
    Outfit Combo 1 → Office wear (uses: Office)
    Outfit Combo 2 → Casual day out (uses: Casual)
    Occasion Section 1 → Everyday wear ← DUPLICATE of Casual — DELETE

OCCASION DIVERSITY REQUIREMENT:
  Each blog must cover at least 4 DISTINCT occasions across both sections combined.
  The following pairs are NOT distinct — treat them as one occasion:
    ❌ "Casual day out" + "Everyday wear"
    ❌ "Running errands" + "Casual outing"
    ❌ "Festive occasion" + "Diwali celebration"
    ❌ "Wedding" + "Bridal look"
    ❌ "Office" + "Professional setting"
    ❌ "Evening" + "Night out"
    ❌ "Beach" + "Vacation"
  If two planned occasions match a pair above → merge into one, replace duplicate with a NEW occasion.

SELF-CHECK BEFORE WRITING SECTION 4:
  Q1: What occasions did I already cover in Section 3? List them.
  Q2: Are any planned Section 4 occasions the same as or similar to those? YES → replace. NO → proceed.
  Q3: Do I have at least 2 genuinely new occasions for Section 4? NO → pick from this list:
    Temple visit / religious gathering | Date night / anniversary dinner | College farewell / graduation
    Baby shower / kitty party | Airport / travel day | Sunday farmers market / flea market
    Office party / team outing | Mehendi / haldi ceremony | Post-workout coffee run | Book club / indoor gathering

FINAL DUPLICATE CHECK — after completing the full blog draft:
  Read every occasion mentioned anywhere. Group similar ones.
  If any group has more than one entry → DUPLICATE FOUND.
  Fix: keep the BETTER written version. DELETE the weaker one entirely. Do NOT merge — DELETE.
  Replace deleted section with a NEW occasion from the unused ideas list above.
...
RULE 9 — NO UNVERIFIABLE SPECIFICATIONS (zero tolerance)
Never state a specific grade, specification, age range, dimension, certification, OR INGREDIENT unless that EXACT phrase appears in the website context or product data provided above.

INGREDIENTS AND FORMULATIONS — most critical for skincare/food/wellness brands:
- Never name a specific active ingredient (silk peptides, hyaluronic acid, retinol, niacinamide, ashwagandha, etc.) unless it appears VERBATIM in the product name or product description in the provided data.
- Never claim a formulation type (whipped, serum, oil-free, gel-cream, etc.) unless stated in the product data.
- Never invent a product name. If the product list contains "Floral Rice Glow Toner" → that is the product. NEVER rename it to "Sandal Bloom Toner" or any other name.
- If the product list is empty or doesn't contain a product matching the blog topic → write about the products that ARE in the list, not an invented version of the topic product.

MATERIAL GRADES — only use what the product name says:
- Product says "Leather Belt" → write "leather belt". NEVER upgrade to "full-grain", "top-grain", "genuine leather" unless the brand uses those exact words.
- Product says "Gold Plated Earrings" → it is gold-plated. NEVER claim silver properties, 925 purity, or sterling silver. Gold-plated and silver are different materials entirely.
- Product says "Silver Plated" → cannot claim 925/sterling silver properties.
- Product says "925 Sterling Silver" → you may state 925 properties. But do NOT apply these claims to other products in the same brand that are NOT marked 925.

AGE RANGES — only use what the brand publishes:
- If the product says "for kids" with no age → write "for children". NEVER invent "ages 4-7" or "suitable for 6-10 year olds".
- If the brand publishes a specific age range → use it exactly as stated.

DIMENSIONS, COUNTS, CERTIFICATIONS — same rule:
- Never state "6 pieces", "200g", "BPA-free", "ISO certified" unless it appears in the product data.
- If unsure → omit the specification entirely. Silence is better than an invented claim.

Fabricating specifications damages brand credibility and exposes the brand to consumer complaints.

RULE 10 — SHIPPING AND POLICY CONSISTENCY
Before writing any FAQ answer about shipping, delivery, or returns, check the website context for any policy information.
If the website context mentions "Free Shipping", "Free Delivery", "COD Available", "Cash on Delivery", or "Free delivery on orders above ₹X":
- The FAQ MUST reflect this correctly. NEVER say "shipping charges vary" if the brand offers free shipping.
- NEVER contradict a stated selling point. Undercutting a brand's free shipping claim in the FAQ damages trust.
- If no shipping information is in the website context → write "Shipping details are available at checkout" rather than inventing a policy.

RULE 11 — SELF-CHECK PIPELINE (run all three categories before outputting)
...
[ ] RULE 9 compliance: no material grade (full-grain, top-grain, genuine, 925, sterling) claimed unless in product data
[ ] RULE 9 compliance: no specific ingredient (silk peptides, hyaluronic acid, etc.) claimed unless in product data
[ ] RULE 9 compliance: no product name invented — every product name in the blog appears verbatim in the product list
[ ] RULE 9 compliance: no age range (4-7, 5-9, 8-12) stated unless published by the brand
[ ] RULE 10 compliance: FAQ shipping answer matches any free shipping / COD claim in website context
[ ] RULE 12 compliance: every section titled "Picks" / "Recommendations" / "Top Picks" contains at least 2 actual product names with prices
[ ] RULE 13 compliance: no internal contradictions (e.g., anti-sugar in one section, pro-sugar product in another without explanation)
[ ] BRAND NAME GRAMMAR: no sentence uses the brand name as a countable noun ("eat a [Brand]", "are [Brand] suitable") — brand name must always be followed by a product noun

CONTENT CHECKS:
[ ] Section 3 occasions used: [list every occasion named in Section 3]
[ ] Section 4 occasions used: [list every occasion named in Section 4]
[ ] Zero overlap between the two lists — even if worded differently (RULE 5)
[ ] At least 4 DISTINCT occasions covered across both sections combined (RULE 5 diversity requirement)
[ ] No pairs from the NOT-distinct list appear across sections (e.g., "Casual day out" + "Everyday wear" = same occasion)
...
RULE 12 — SECTION PROMISE ENFORCEMENT
If a section is titled "X Picks", "Our Top Picks", "Recommended Products", "Best Options", or similar → it MUST contain at least 2 actual product names and prices from the product list inline in that section. A section with that title that contains only generic advice and no product names is a broken promise to the reader.
WRONG: A section titled "Halesnax for Pre-Yoga Energy: Our Top Picks" that contains only "Halesnax offers several wholesome options... our snacks are crafted with traditional wisdom..." with no product names.
RIGHT: The section names 2–3 specific products with prices. If you cannot name products in that section, rename the section to something that doesn't promise picks (e.g., "Pre-Yoga Snacking Principles").

RULE 13 — NO INTERNAL CONTRADICTIONS
Before outputting, check: does any advice in the blog contradict any other advice?
Common contradiction pattern: blog recommends "low sugar" in one section, then recommends a high-sugar product for the same use case in another section. If there's a legitimate reason the advice changes (e.g., pre-workout vs post-workout glycogen needs) → explain the reason explicitly. Don't leave contradictions unexplained.

WHAT TO DO IF INPUTS ARE INSUFFICIENT:
If the product list has fewer than 3 products matching the blog topic → use whatever products ARE available. Write the best possible product section with what you have. Do NOT stop. Do NOT output an error message.
If the product list is empty → write a general educational blog without naming specific products. Do NOT invent product names.
If prices are missing for products → omit the price rather than inventing one. Write "(price available on site)" only if needed.
Always produce a complete blog. An error message or a stopped output is never acceptable.
...
If fewer than 3 products pass CHECK 5 → use whatever matching products ARE available (even 1 or 2).
If ZERO matching products exist for the exact type (e.g., no "Necklace" products) → use the closest related products from the same parent category (e.g., for a necklace blog, use Pendant or Chain products if available).
If ZERO products exist in the entire parent category → write the product section as an educational recommendation ("look for these features when choosing...") without naming specific products, and add a note: "Visit [Brand]'s collection to see current availability."

Do NOT output an ERROR message. Do NOT stop blog generation. Always produce a complete blog — the product section adapts to whatever inventory is available.
...
## STEP 2 — PRICE LOCK + TIER DETECTION (Non-negotiable)
...
${tierMap ? `CATALOG-DERIVED PRICE TIERS (computed from real SKUs — use ONLY these tier headings):
${tierMap}
RULE: Do NOT write any price-tier H2 that is not in the list above. If a tier has fewer than 2 products — skip it entirely. If all products fit one tier — use a single "[Brand] picks" section with no price-tier framing.` : "Do not create price-tier H2 sections unless you have at least 2 real products for each tier."}

${materialConstraints ? `---\n${materialConstraints}\n---` : ""}

${urgencyLine ? `URGENCY LINE (inject into the product section header AND the closing CTA):
"${urgencyLine}"
Both locations must contain this exact line. Do not paraphrase it.` : ""}
...
## STEP 3.5 — TOPIC CATEGORY LOCK

Read the TOPIC CATEGORY CONSTRAINT line in the WEBSITE CONTEXT above.

If a TOPIC CATEGORY CONSTRAINT is present:
- You are writing about ONE specific product type only (e.g., "hoops", "studs", "tote bags").
- Every product you recommend in Section 7 (or the gifting Product Cards section) MUST be of this exact type.
- If a product in the list is a DIFFERENT type within the same category (e.g., stud earrings when topic is hoops) → REJECT it. Do NOT include it, even if it's a beautiful or popular product.
- Do NOT make body claims about product types that are not in your product list. If your product list has only hoops → do NOT write "discover our full earring range" or "browse our studs and drops".
- Be specific in the body: if you are writing about hoops, say "hoops" — not "earrings" (too broad).

If NO TOPIC CATEGORY CONSTRAINT is present → proceed normally.

---

## STEP 4 — WRITE THE BLOG

${(contentType || "").toLowerCase().includes("gift") ? `
🎁 GIFTING / PRODUCT-DISCOVERY BLOG DETECTED (content type: ${contentType})
Use the CONVERSION-FIRST template below — NOT the 8-section educational structure.

GIFTING BLOG STRUCTURE (5 sections):

### SECTION 1 — HOOK (≤80 words)
Drop the reader into a specific gifting moment — not the generic occasion, but the exact situation a buyer is in right now.
GOOD: "Father's Day is 10 days away and you still haven't found something that doesn't feel impersonal. A leather bag lasts three years. A gift card is forgotten in two weeks. One of these is actually a decision — the other is a placeholder."
BAD: "Father's Day is the perfect time to show your appreciation. Finding the right gift can be challenging."
The hook must create a specific scene, not state the obvious.

### SECTION 2 — GIFT PICKS (must appear within the first 30% of total word count)
🚨 THIS IS THE ONLY PLACE PRODUCT NAMES AND PRICES APPEAR.
${urgencyLine ? `Add this line at the TOP of this section: "${urgencyLine}"` : ""}

For each product, write:
**[Exact Product Name]** — ₹[Exact Price]
[Sentence 1: One specific physical detail that makes this worth giving — material, feature, or design point]
[Sentence 2: Who specifically should give this, and to whom — name the relationship or use case]

GOOD example:
**Full-Grain Leather Tote** — ₹3,200
The unlined interior ages with use — the leather develops a patina that makes each piece look more personal over time, not less.
The right choice if you're gifting someone who uses a bag for work and wants something that looks better in year three than year one.

BAD example (never produce this):
**Full-Grain Leather Tote** — ₹3,200
This beautiful bag is the perfect gift for someone special. It will be loved and cherished for years to come.

### SECTION 3 — BUYER'S GUIDE
2–3 practical tips for choosing between the products above. Address the actual decision anxiety: price vs. practicality, occasion-appropriateness, size, longevity. Make the decision easier, not harder. No generic "consider your budget" advice.

### SECTION 4 — FAQ (minimum 3 questions)
${policyContext ? `Use the brand policy excerpts below to write AT LEAST 2 brand-specific Q&As (shipping time, returns, care, or customisation). The other 1–2 may be category/product-specific. Zero purely generic FAQs.

BRAND POLICY EXCERPTS:
${policyContext}` : "Write Q&As a real gift-buyer would search for. At least 1 must be about delivery/shipping timing relevant to the occasion. Each answer: 2 prose sentences, 40–60 words."}

Each answer: exactly 2 prose sentences, 40–60 words. No bullet lists inside answers.

### SECTION 5 — CTA
Warm, helpful close pointing to the collection.
${urgencyLine ? `Must contain: "${urgencyLine}"` : ""}
Brand name must OPEN the CTA sentence — not be buried at the end.

` : `
Use this 8-section structure. Do not skip any section.
${policyContext ? `\nBRAND POLICY EXCERPTS (use for Section 6 FAQ brand-specific Q&As):\n${policyContext}\n` : ""}`}
...
---
## WRITING QUALITY STANDARDS — READ BEFORE WRITING A SINGLE WORD

⚠️ CRITICAL: The format descriptions below show HOW to write — they are structural guides only. NEVER copy any example wording into the blog. All content must come from the brand's actual products and category. Copying examples from this section into the blog is a critical failure.

**STANDARD 1 — THE SPECIFICITY TEST**
Before writing any sentence, ask: "Could this sentence apply to any product in any category?" If yes — delete it and write something specific to THIS brand's actual product.
WEAK pattern: "[Product] is perfect for everyday use and special occasions." — could describe anything
STRONG pattern: Name a specific physical feature of the product (texture, weight, size, ingredient, material) and explain exactly why that feature matters in a real-use situation.
The strong version proves the writer looked at the actual product.

**STANDARD 2 — SENTENCE RHYTHM**
Never write three sentences of the same length in a row. Mix short declarative statements (5–8 words) with longer explanatory sentences (15–20 words). Short sentences land. Longer sentences carry the nuance. Then a short one again.

**STANDARD 3 — THE EARNED ADVICE TEST**
Good advice sounds like it came from someone who made the mistake first.
WEAK pattern: Generic positive recommendation with no friction or specificity.
STRONG pattern: Name the common mistake people make → then give the correct approach → explain WHY the correct approach works. The mistake-first structure makes advice memorable and credible.
Apply this to every tip, styling suggestion, or recommendation.

**STANDARD 4 — ZERO GENERALISATION RULE**
Any sentence that would be equally true if you replaced the brand name with any competitor's name must be deleted or rewritten.
DELETE patterns: "[Brand] offers a wide range of quality products at affordable prices" / "Made with care for every family"
KEEP patterns: Sentences naming a specific ingredient, process, certification, or product feature that genuinely differentiates this brand from competitors.

**STANDARD 5 — INDIAN BUYER CONTEXT**
Indian buyers think in specific ways. At least ONE of these must appear in the blog:
- VALUE CALCULATION: Give buyers the cost-per-use math. "At ₹[price], that works out to ₹[X] per [use/day/serving]" is more persuasive than "affordable" or "value for money."
- DAILY VS. OCCASION: Indian buyers distinguish sharply between daily-use and special-occasion products. Address where this product sits and what that means for the buyer.
- GIFTING DYNAMICS: If this product is gifted, name who typically buys it and for whom. Mother for child? Couple? Corporate? Each changes the product choice.
- REGIONAL FIT: If this product is consumed or used differently across Indian regions — name it. If not relevant — skip.

**STANDARD 6 — BRAND DIFFERENTIATION RULE**
Somewhere in the blog, write ONE sentence answering "Why choose [Brand] over any other option?" Use only information from the brand audit — never invent. The sentence must be specific, not a generic claim.

---

### SECTION 1 — HOOK

The hook must use ONE of these three structures. Write original content — DO NOT copy the structure descriptions below:

**A — SCENE:** Open with a specific moment in the reader's life where this product solves a real problem. Set the physical context, the situation, the tension. Under 80 words.

**B — COUNTERINTUITIVE CLAIM:** Open with something that contradicts what most people assume about this product category. Must be backed by a real fact or insight from the brand's category — not a generic surprise.

**C — SPECIFIC OBSERVATION:** Open by naming one precise detail about this product that most people don't know — something that reveals category expertise.

The hook must NOT:
- Ask a question ("Are you looking for...", "Have you ever wondered...")
- State the obvious about the product category
- Use any banned opener
- Mention the brand name in the first sentence

### SECTION 2 — CATEGORY AUTHORITY
2–3 sentences proving the writer knows this specific product category. Must include one detail that separates expert knowledge from a Google summary. The detail should name a mechanism, tradeoff, or fact that most buyers don't know but would want to.
Write this entirely from the product category in the blog — never from any other category.

### SECTION 3 — USE-CASE COMBINATIONS (minimum 3)
Each combination must answer: WHAT to use/pair it with + WHEN/WHERE + one specific reason it works OR one mistake to avoid.
Each should feel like advice from someone who has actually used the product in that situation — not from someone who imagined it.
...
🚨 NO DUPLICATE OCCASIONS (RULE 5 — STRICT):
BEFORE writing this section, run the RULE 5 self-check:
  Step 1: List every occasion already named in Section 3.
  Step 2: Are any of your planned Section 4 occasions on that list? YES → replace with a new occasion from the unused ideas list in RULE 5. NO → proceed.
  Step 3: Do you have at least 2 genuinely NEW occasions that are not in Section 3? NO → pick from the RULE 5 unused occasions list.

What counts as a DUPLICATE (even if worded differently):
  "Casual day out" in Section 3 + "Everyday wear" in Section 4 → DUPLICATE — delete the Section 4 one.
  "Office wear" in Section 3 + "Professional setting" in Section 4 → DUPLICATE — delete the Section 4 one.

If Section 3 has already covered all relevant occasions for this brand → Section 4 is skipped entirely. Do NOT add filler paragraphs.

The CORRECT structure (follow this model):
  Section 3: Office Chic | Bohemian Weekend | Evening Glamour
  Section 4: Wedding Guest | College Farewell  ← genuinely new, zero overlap

The WRONG structure (never produce this):
  Section 3: Office wear | Casual day out
  Section 4: Professional setting ← DUPLICATE | Everyday wear ← DUPLICATE
...
3–5 tips. Every tip must name a SPECIFIC mistake real buyers of THIS product make and how to fix it. Write entirely from this product category — never from another category.
FORMAT (mandatory): State the common mistake first → then the correct approach → then why it matters.
WRONG pattern: Generic care or usage advice that applies to any product in any category.
RIGHT pattern: A mistake specific to buying or using THIS type of product, backed by a real reason grounded in how the product works.

### SECTION 6 — FAQ (MANDATORY — minimum 3 questions, maximum 5)

This section is REQUIRED. Do not skip it. Do not merge it into another section.

Write 3–5 Q&A pairs. Each question must be something a real buyer of THIS product would genuinely search for. Each answer: exactly 2 flowing prose sentences, 40–60 words total. No bullet points inside answers.

AIM for this quality mix (not a hard requirement — just a guide):
- 1 question about product care, durability, or material behaviour for this specific product
- 1 question about fit, size, or practical use (a real physical concern)
- 1 question about shipping, returns, or buying process ${policyContext ? `— use these policy details: ${policyContext.slice(0, 300)}` : ""}

GOOD questions (these feel real and specific):
  "Will this anklet tarnish if I wear it daily in Mumbai humidity?"
  "My wrist is 7.5 inches — will the clasp reach?"
  "Can I wear the same anklet on both feet or does it look overdone?"

AVOID questions so generic any brand could answer them:
  "How do I care for silver jewellery?" — too broad
  "Is jewellery in fashion?" — not a buyer concern
  "What is 925 silver?" — educational filler, not a buyer question

Format for each Q&A:
**Q: [Question]**
[Answer in exactly 2 sentences, 40–60 words total, flowing prose.]

### SECTION 7 — PRODUCT RECOMMENDATIONS (ALL products and prices go HERE only)
🚨 THIS IS THE ONLY PLACE IN THE BLOG WHERE PRODUCT NAMES AND PRICES APPEAR. No product names or prices in Sections 1–6.

For each product, write in this EXACT format:

**[Exact Product Name from catalog]** — ₹[Exact Price from catalog]
[Sentence 1: ONE specific physical or design detail that distinguishes this product from the others in this list — texture, material, chain weight, clasp type, stone cut, anything concrete and observable.]
[Sentence 2: WHO this product is specifically for — name the reader type, use case, or the decision moment where this becomes the obvious choice over the others.]

EXAMPLE OF REQUIRED QUALITY:
**925 Sterling Silver Orbit Anklet** — ₹1,250
The box chain lies completely flat against the ankle — no twist, no movement, which means it won't catch on shoe straps or shift position through the day.
If you want a single anklet that reads as intentional across both ethnic and western outfits without any adjustment, this is the one to start with.

EXAMPLE OF REJECTED QUALITY (delete and rewrite if you produce this):
**925 Sterling Silver Orbit Anklet** — ₹1,250
This beautiful anklet is perfect for daily wear and special occasions. It will make a wonderful addition to any jewellery collection.

The second version says nothing. It could describe any product. It will not help a reader choose. Rewrite until every product description could only belong to that specific product.

Product rules:
- Use EXACT product names — character-for-character from the product list
- Use EXACT prices — ₹1,250 stays ₹1,250, never ₹1,200 or "around ₹1,200"
- Only include products confirmed in the product list — no invented products, ever

### SECTION 8 — CTA

The CTA must do three things: reference 1–2 specific products by name, give a concrete reason to visit the site NOW, and open with the brand name.

EXAMPLE OF REQUIRED QUALITY:
"[Brand] has both the [Product A] and [Product B] ready to ship — if you've been deciding between them while reading this, the [Product A] is the safer first choice for daily wear, and the [Product B] is the one you'll reach for more. Both under ₹1,200."

EXAMPLE OF REJECTED QUALITY (never produce this):
"Explore [Brand]'s stunning collection of silver jewellery and find the perfect piece for every occasion. Shop now and elevate your style today."

The second version says nothing specific. It could belong to any brand, any product, any blog. It will not convert.

RULES:
- Open with the brand name — not buried at the end
- Reference at least 1 product from Section 7 by name
- Give the reader a reason to click that is specific to this blog's topic and products
- No urgency clichés: "Don't miss out", "Shop before it's too late", "Limited stock"
- ${urgencyLine ? `Include this shipping urgency line naturally: "${urgencyLine}"` : "If there is a genuine time-sensitive reason (festival, limited stock), state it plainly — not as a pressure tactic"}
- Brand name must appear 3–4 times naturally across the full blog body — not just in the CTA

---

## STEP 5 — RULE 11 SELF-CHECK PIPELINE (Run all three categories before outputting)
...
### CONTENT CHECKS
[ ] Section 3 occasions used: [list internally]
[ ] Section 4 occasions used: [list internally]
[ ] Zero overlap between Section 3 and Section 4 occasions (RULE 5)
[ ] RULE 6: no closed-toe shoe pairings for foot jewellery
[ ] RULE 7: brand identity vocabulary matches brand type
[ ] Every FAQ answer: exactly 2 sentences, 40–60 words, prose only
[ ] No FAQ questions that pass the Google test (generic questions answerable without brand knowledge)
[ ] Hook uses Scene / Counterintuitive Claim / Specific Observation — not a question or obvious statement
[ ] SPECIFICITY CHECK: Read every paragraph. Does any sentence apply equally to any other brand or product? → DELETE it and replace with something specific
[ ] EARNED ADVICE CHECK: Does every styling tip name a specific mistake or a concrete physical detail? → If it's generic advice, rewrite it
[ ] BRAND DIFFERENTIATION: Does the blog contain at least one sentence explaining why THIS brand specifically? → If not, add it
[ ] INDIAN BUYER CONTEXT: Does the blog address the daily/occasion split, cost-per-wear, or gifting dynamics? → At least one must appear
[ ] Product section: each product has one specific physical detail AND one specific buyer type — not generic praise
[ ] CTA: references 1–2 specific products by name, gives a concrete reason to visit, opens with brand name
[ ] No banned phrases anywhere in the blog
[ ] "Alippo" absent from entire blog — replaced with actual brand name
[ ] No prompt artifact headings anywhere
[ ] Blog ends with complete CTA — not mid-sentence
...
- ## for H2 headings, ### for H3 — never skip a level
- Bold ONE concrete, specific line per section — not a generic claim, the single most useful sentence
- 1 blank line after every heading, 1 blank line between every paragraph
- Every paragraph: 3–5 sentences of connected prose — NEVER one sentence per line
- Length: match the content need. Gift guides: 700–900 words. Educational/buying guides: 1,000–1,300 words. Care guides: 600–800 words. Do NOT pad to hit a word count.
- Simple English — write for a regular Indian reader reading on their phone. Max sentence length: 20 words.

BANNED PHRASES — zero uses anywhere (delete on sight and replace with specific language):
- "In today's world" / "In conclusion" / "It is important to note" / "Look no further"
- "Ever wondered" / "You're not alone" / "Most people don't realize" / "Dive into"
- "adding a touch of elegance" / "elevate your entire look" / "exudes confidence" / "perfect for special occasions"
- "true testament" / "stands as a testament" / "is a testament to"
- "meticulously crafted" / "enduring style" / "lasting appreciation" / "tells its own story"
- "unmatched versatility" / "investment in lasting style" / "deeply meaningful" / "truly memorable"
- "elevates it beyond mere utility" / "speaks volumes" / "says it all" / "checks all the boxes"
- "pairs beautifully with anything" / "subtle yet statement" / "effortlessly chic" / "fashion-forward"
- "Whether you're X or Y" / "From X to Y" / "No matter the occasion" / "For every woman"
- Any sentence that could belong to any brand selling any product in any category → DELETE

VOCABULARY CAPS — count before outputting. If exceeded, replace with the specific product/material/use-case detail that earns the adjective:
- "premium" → max 2 uses (what specifically makes it premium? say that instead)
- "elegant" / "sophisticated" → max 1 use each
- "timeless" → max 1 use
- "crafted" / "luxurious" / "refined" / "elevated" → max 2 uses each
- "exquisite" / "meticulously" / "impeccable" / "outstanding" → max 1 use total across all four

DO NOT stop mid-blog. Write until the CTA is complete. An incomplete blog is a failed output.
`.trim();



// ─── Density / Padding Tighten Pass ──────────────────────────────────────────
export const PROMPT_TIGHTEN = (blogText) => `
You are a ruthless blog editor. Your job is to cut 25–35% of the words from this draft without losing any information.

WHAT TO CUT:
- Sentences that restate the previous sentence in different words — delete the weaker one entirely.
- Dependent clauses that add no new information (e.g. "which is important to keep in mind", "as we mentioned earlier").
- Filler openers: "It is worth noting that", "In fact,", "Of course,", "Naturally,", "It goes without saying".
- Redundant adjective stacking: "beautiful and elegant" → "elegant". "durable and long-lasting" → "durable".
- Padding transitions: "Now that we've covered X, let's move on to Y" → just start Y.
- Any sentence whose removal leaves the meaning fully intact.

WHAT TO KEEP EXACTLY:
- All product names, prices, and product descriptions.
- All headings (H1, H2, H3) — do not remove or reword headings.
- All FAQ questions and answers — trim if possible, but keep substance.
- All CTAs and urgency lines.
- All factual claims about materials, care instructions, or occasions.
- The hook (first paragraph) — can trim but must keep the opening scenario.

OUTPUT RULES:
- Return the tightened blog only. No preamble. No commentary.
- Start directly with the H1 title or Meta Title if present.
- Do not add new content. Only remove and tighten existing content.
- Target: 25–35% word reduction. Do not go below 700 words total.
