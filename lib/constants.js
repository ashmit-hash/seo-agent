...
1. For every product name in the Blueprint: look up by name (case-insensitive, whitespace-normalised) in product_catalog. No match → block: hallucinated_product.
2. Check for repeat-digit patterns (e.g. "925925") in product names → block: concatenation_artifact.
3. If the same product appears 3+ times in batch_history → warn: product_overused.
4. Count successfully resolved products. If fewer than 3 → block: insufficient_catalog_match.
5. If Blueprint references a product class not in catalog (e.g. "necklaces" when catalog only has earrings) → block: product_class_mismatch.
